...
**`main` is at v0.46.0 — the app is usable on an Android phone, not just an iPad (2026-08-21).** The bodies of every screen already laid out correctly at 412px; what was broken was the **header bars**, built as single non-wrapping rows for an iPad and running up to 180px off a phone screen with "School Info" and "Light Survey" simply absent and no sign anything was missing. On the Assessment screen that also crowded the hamburger that is the *only* way to reach another zone. Fixed by collapsing the extra actions behind a "More" menu below `sm`; above `sm` every screen renders exactly as before.
