...
**`main` is at v0.40.0 (`491989a`) — the report is signed with the date it was signed (deployed 2026-08-19).** Three releases, driven by the first night of real light-survey data off the meter: the **0.1 fc uniformity floor** (v0.38.0), a **plain-language intro** to the lighting section (v0.39.0), and a **`report_signed_on` field** so a multi-visit assessment isn't signed with its walk date (v0.40.0). Git and production are back in sync — the rebuilt bundle hash matches the served one exactly.
...
- **Light surveys and the aerial picker: still nothing tested on an iPad.** GPS corner capture, the share-sheet KML hand-off, the screenshot upload, the sync round trip, and the whole overscan gesture rework are desktop/headless-verified only. Multi-file (>99 reading) imports still not built.
