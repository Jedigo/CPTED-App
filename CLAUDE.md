...
**`main` is at v0.33.3 — lot corners are picked by tapping county aerial imagery (deployed 2026-08-18).** Three releases in one day built the light-survey feature (v0.31.0), synced it to the server (v0.32.0), and then replaced its weakest step: placing a lot's corners. See the three 2026-08-18 session-log entries.
...
- **URGENT — photo GPS now records, and on these iPads it may be badly wrong.** Enabling HTTPS switched on `getGPSCoordinates()`, which had been failing silently for all 1,066 photos on the server. The field iPads are **Wi-Fi-only (no GNSS)**, so a "location" is a Wi-Fi lookup; tethered to a phone hotspot in a parking lot it can be off by hundreds of feet while looking authoritative. Wrong-but-plausible coordinates in a Sheriff's Office report are worse than none. **Fix before the next field day: store coordinates only when `coords.accuracy` is good, otherwise store null.** Proposed 2026-08-18, not built.
- **Map pan/zoom still lags** on the aerial corner picker — the "Updating…" pill sits for roughly half a second on every gesture while the county server renders a new image. Live drag/pinch transforms and an in-memory view cache took the worst of it (v0.33.1–v0.33.3), but the fetch is still on the critical path. Next lever: fetch a wider area than the screen shows (overscan) so short gestures need no request at all, or pre-fetch the neighbouring zoom level. Deferred 2026-08-18 — good enough for a v1 field pass.
