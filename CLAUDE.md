...
**`main` is at v0.43.0 — draggable lot corners on the aerial, and every assessment now says which copy is newest (deployed 2026-08-20).** Three releases. **v0.41.2** names the standard the light survey is judged against: 3.0 fc and 4:1 are **CPTED crime-prevention levels from NICP training, not code** — and stricter than the code a principal answers to (Florida Building Code 453.10.3.5 asks 1.0 fc for school parking), so a lot can satisfy its code and still read "Below". No arithmetic changed. **v0.42.0** adds per-assessment **revision tracking** for the shared iPads. **v0.43.0** makes the lot rectangle **draggable** on the county aerial.

**The next thing to work on: getting the working grid into the field.** See the To-Do below — the Google Earth round trip did not survive contact with a real night walk.
...
- **NEXT UP — the working grid needs to be usable *in the field*, on the device doing the walk.** Raised 2026-08-20 after a real night walk: the current route is export KML → open in Google Earth → read it on a *second* device, or screenshot it. Neither survived contact with the job. The assessor is walking a dark car park holding a meter, and needs to know **where point 37 is** and **which point they are on**, without juggling two devices or squinting at a static screenshot.
  - What exists to build on: `gridPointLatLng()` already gives every point's real coordinate, `AerialCornerPicker` already draws the lot on county aerial imagery in-app with both directions of the projection, and `aerial-render.ts` already composites the grid onto that imagery for the report. So an **in-app walking view** — the aerial, the numbered grid, and the current point highlighted — needs no new data and no new imagery source.
  - The awkward part is **"where am I"**: the iPads are Wi-Fi-only with no GNSS, so a blue dot is not available (15–30 ft error at best, and that was what killed GPS corner capture). So the walking view probably has to be **tap-to-advance** — the assessor taps "next" as they take each reading and the highlight moves along the serpentine — rather than positioning them automatically.
  - Worth deciding early: whether the app should also **capture the reading** at each point (typed in as they go) rather than importing the meter file afterwards. That would remove the whole SDL400 file transfer, the 99-position cap, and the multi-session merge problem below — at the cost of typing in the dark. Ask the assessors which they would actually rather do.
  - KML export should stay either way; it is useful at the desk and for anyone who wants it.
- **Multi-session meter files are not importable.** Exiting and re-entering the SDL400's manual mode appends a *second session to the same file*, marked by a repeated header row and restarting at P1. The parser only treats row 0 as a header, so the second one is swallowed as "1 row could not be read" and every session is merged by position number, last-wins — lot 3 silently overwriting lots 1 and 2. Decided 2026-08-18 **not** to build a session picker: it would confuse the other assessors mid-import. Field workaround: copy each lot's file off the card **and delete it from the card** between lots, so each lot gets its own file. Timestamps separate sessions if that is ever missed.
...
- **Light surveys and the aerial picker: still nothing tested on an iPad.** The v0.43.0 drag interaction is sized for a fingertip but has only ever been driven by a mouse — `HANDLE_GRAB_PX` (26px) in `AerialCornerPicker.tsx` is the constant to tune after a real session, and the magnifier's placement is the other thing to watch. GPS corner capture, the share-sheet KML hand-off, the screenshot upload, the sync round trip, and the whole overscan gesture rework are desktop/headless-verified only. Multi-file (>99 reading) imports still not built.
