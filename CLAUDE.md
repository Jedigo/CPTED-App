...
- **Phones are supported too, as a secondary device** (v0.46.0). Everything below Tailwind's `sm` (640px) is phone territory: headers collapse their extra actions behind a "More" menu via `HeaderActions.tsx`, and tap targets grow to 44px. Above `sm` nothing changes, so an iPad layout is never affected by a phone fix. Two exceptions, deliberate: the **aerial corner picker** is a desk task and is not sized for a phone, and generating a PDF on one is untested.
...
**`main` is at v0.47.0 — the GPS corner-capture buttons are gone (2026-08-21).** They shipped in v0.31.0 when pasting from Google Maps was the only alternative; the aerial picker (v0.33.0) beat them roughly twenty to one, and v0.44.0's cached imagery removed their last argument. They were the easiest control on the screen and their failure was invisible — a lot of the wrong size, a grid laid out on it, and every reading attributed to the wrong ground with nothing looking broken. **Photo geotagging keeps its GPS and its 30 m gate:** recording which property you were at is a different job from measuring how big it is.

**v0.46.0 — the app is usable on an Android phone, not just an iPad (2026-08-21).** The bodies of every screen already laid out correctly at 412px; what was broken was the **header bars**, built as single non-wrapping rows for an iPad and running up to 180px off a phone screen with "School Info" and "Light Survey" simply absent and no sign anything was missing. On the Assessment screen that also crowded the hamburger that is the *only* way to reach another zone. Fixed by collapsing the extra actions behind a "More" menu below `sm`; above `sm` every screen renders exactly as before.

**Earlier the same day, v0.45.0 — the lighting grid is now walked in the app, and readings can be typed at the point (deployed + pushed 2026-08-21).** Two releases, closing the task the 2026-08-20 night walk raised. **v0.44.0** is a full-screen **walking view** at `/assessment/:id/light/:surveyId/walk`: the county aerial, the lot outline, every point numbered, the current one highlighted, and Previous/Next at 68px. **v0.45.0** adds a **keypad** beside the map — read the meter, type the value, tap Record, and it saves against the point and steps on. The meter-file import is untouched and stays.
...
- **Photo geotagging has never once worked in production and is now testable.** 1,066 photos, zero coordinates — geolocation was blocked by the insecure origin until 2026-08-18, and the iPads have no GNSS anyway. An Android phone on the **HTTPS** origin has both, so photos should carry coordinates for the first time. Untested. Note IndexedDB is per-origin: sync on the old URL before moving a device to the HTTPS one.
- **The aerial corner picker is not usable on a phone**, and was left that way on purpose (v0.46.0). Framing a lot means dragging corners on a map at roughly half a foot per pixel, where `HANDLE_GRAB_PX` is 26px and a fingertip covers ~44px — a much larger slice of a 412px screen than of an iPad. That is a design problem rather than a layout fix, and grids are plotted at a desk anyway. Revisit only if someone actually needs to frame a lot from a phone.
- **Generating a PDF on a phone is untested.** The merged-PDF path already routes through the share sheet on iOS; nobody has tried either path on Android.
...
### 2026-08-21 (later still) — GPS Corner Capture Removed (v0.47.0)

- **Prompted by a fair question: if GPS can be that far off, why are the buttons still there?** The honest answer was that nobody had re-justified them since the aerial picker was built. They shipped in **v0.31.0** when the only alternative was pasting from Google Maps; **v0.33.0**'s picker beat them by roughly twenty to one (about half a foot per pixel against 10–30 ft); **v0.44.0**'s cached imagery removed their last argument, offline use. **v0.32.2** came closest to a decision but only made a bad option *visible* — it added the accuracy readout and the warning.
- **Removed rather than fixed, because the failure is invisible.** It was the easiest control on the screen — one tap while standing there, against framing a box — and a bad fix produces a lot of the wrong size, a grid laid out on it, a completed walk, and every reading pinned to the wrong ground with nothing looking broken. On a 66 ft-wide lot a 20 ft error is a 30% error in the width. The v0.42.0 rule applies: a warning people learn to tap through fails on the one occasion it is right.
- **Every case it covered is covered better**, including the awkward one. Unplanned lot with no cached imagery and no signal: **pacing it and typing the length and width** beats a handheld fix. The remaining routes are now stated on screen ranked by accuracy rather than by convenience — map ±6 in, pasted coordinates ±3 ft, paced dimensions a foot or two.
- **Photo geotagging keeps `navigator.geolocation` and its own 30 m gate.** The distinction is now written into the code so the buttons do not come back: **GPS is fine for recording where you were and useless for measuring how big something is.** Same sensor, different tolerance.
- **Correction worth recording:** the corner button's warning threshold was a *local* `GPS_ACCURACY_LIMIT_M = 5` inside `LightSurveyDetail.tsx`, shadowing the unrelated 30 m constant of the same name exported from `photos.ts`. Two constants, one name, different values, six-fold apart. The removal takes the duplicate with it.
- Verified with 8 checks: the button and its readout are gone at both desktop and phone width (gone, not merely hidden on one), the screen explains why, and **pasting three corners still sizes the lot** — the route that replaced it. All five existing suites still pass (95 checks) and every screen still reports 0px overflow.

### 2026-08-21 (later) — Phone Layout (v0.46.0)

- **Driver: the app was opened on an Android phone and the layout did not fit.** Measured at 412 x 915 rather than guessed, and the result was narrower than expected.
- **The bodies were already fine.** Item cards, Yes/No/UTO buttons, notes, Summary cards, the waiver, and the whole walking view lay out correctly at phone width — the walking view because it was built with orientation variants the day before, the rest because they are single-column card stacks and Tailwind's defaults do the right thing.
- **The breakage was entirely in the header bars.** Built as single non-wrapping rows for an iPad: Assessment carries seven controls and ran **180px off the screen**, so "School Info" and "Light Survey" were absent with nothing indicating anything was missing, and the hamburger — the only route to another zone on a phone — was crowded to the edge. Home clipped "+ New Assessment" and the "Server" tab; Summary overlapped the theme toggle onto the back button.
- **Fixed with one new component, `HeaderActions.tsx`.** Above `sm` every action renders inline exactly as before, so **no iPad layout changes at all**; below it, `keep`/`primary` actions stay on the bar and the rest move behind a "More" menu. The action list is declared once and rendered twice from the same data — an inline row and a menu that drift apart is how something ends up reachable on one device and not the other. A `shortLabel` keeps "+ New Assessment" intact on the iPad while the phone shows "+ New".
  - **Two wrapped rows of buttons was the alternative and was rejected**: ~140px of vertical on a screen with none to spare.
- **Found while verifying: `hidden sm:inline-flex` on `HeaderBackButton` had never worked.** Tailwind emits `.hidden` *before* `.inline-flex` (char 11518 vs 11597 in the built stylesheet), both plain utilities of equal specificity, so the component's own base `inline-flex` won on source order regardless of what the caller passed. The "Home" button the Assessment header meant to hide on small screens had been showing all along. Replaced with an `iconOnlyOnPhone` prop so the decision lives where the base class does and cannot be silently overridden again.
- **Also fixed:** the device-name button on Home was styled `text-ink/50` on a navy bar — dark text on dark navy in light mode, effectively invisible. It now goes through `HeaderActions` and inherits the same styling as everything else.
- **Tap targets:** the theme toggle (36px), the icon-only back buttons, and the walkthrough-phase tabs (40px) all grow to at least 44px below `sm` and are unchanged above it. Every screen now reports **0px overflow and zero sub-44px targets**.
- **Method note: the first audit run measured a blank page and reported a clean bill of health.** The seeded `item_scores` omitted `photo_ids`, `ChecklistItem` read `.length` on it, and the screen threw — so "0px overflow" meant "nothing rendered". The audit now fails loudly when a page renders almost no text. Same family as the tap-to-jump test that skipped itself the day before.
- Verified with an overflow/tap-target audit across six screens plus an 18-check functional run: the menu opens, closes on an outside tap, its items navigate, the zone drawer opens and lists zones — and at 1024px every action is still inline with **no menu present at all**. The three walking-view suites still pass unchanged (113 checks).
- **OPEN:** the aerial corner picker is deliberately still not phone-usable (a design problem, not a layout one, and plotting is a desk task), and generating a PDF on Android is untested.
