...
**`main` is at v0.37.0 — light-survey grids read the centre of each cell, so every cell is a true square (deployed 2026-08-18).** Six releases in one day: the light-survey feature itself (v0.31.0), server sync (v0.32.0), the county-aerial corner picker (v0.33.3), the photo-GPS accuracy gate (v0.34.0), aerial map overscan (v0.35.x), and two passes on grid geometry (v0.36.0, v0.37.0). See the four 2026-08-18 session-log entries.
